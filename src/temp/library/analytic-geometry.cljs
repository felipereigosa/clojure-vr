(ns temp.library.analytic-geometry
...
(defn sphere-plane-intersection [ps rs [a b c]]
  (let [v1 (vector/subtract b a)
        ...
        k (/ (vector/dot-product (vector/subtract a ps) vn) (Math/pow (vector/length vn) 2))]
    (if (> (Math/abs k) rs)
      nil
      (let [pc (vector/add ps (vector/multiply vn k))
            rc (Math/sqrt (- (* rs rs) (* k k)))]
        [pc rc]))))
...
(defn get-normalized-plane-point [plane point grain]
  (let [[a b c] plane
        ...
        s (* grain (Math/round (/ s grain)))
        t (* grain (Math/round (/ t grain)))]
    (reduce vector/add [a
...
(defn line->plane [[point direction]]
  (let [direction (vector/normalize direction)
        v (if (util/float= (Math/abs (vector/dot-product direction [1 0 0])) 1.0)
            [0 1 0]
            [1 0 0])
